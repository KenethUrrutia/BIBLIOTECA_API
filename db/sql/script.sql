-- Active: 1720219855137@@192.168.0.8@5053
...
CREATE TABLE IF NOT EXISTS `BIBLIOTECADB`.`LIBRO` (
    ...
    PRIMARY KEY (`IDLIBRO`),
    CONSTRAINT `fk_LIBRO_GENERO` FOREIGN KEY (`IDGENERO`) REFERENCES `BIBLIOTECADB
`.`GENERO` (`IDGENERO`) ON DELETE NO ACTION ON UPDATE NO ACTION
);
...
CREATE TABLE IF NOT EXISTS `BIBLIOTECADB`.`PRESTAMO` (
    ...
    CONSTRAINT `fk_PRESTAMO_USUARIO1` FOREIGN KEY (`IDUSUARIO`) REFERENCES `BIBLIOTECADB
`.`USUARIO` (`IDUSUARIO`) ON DELETE NO ACTION ON UPDATE NO ACTION,
    CONSTRAINT `fk_PRESTAMO_LIBRO1` FOREIGN KEY (`IDLIBRO`) REFERENCES `BIBLIOTECADB
`.`LIBRO` (`IDLIBRO`) ON DELETE NO ACTION ON UPDATE NO ACTION,
    CONSTRAINT `fk_PRESTAMO_ESTADO1` FOREIGN KEY (`IDESTADO`) REFERENCES `BIBLIOTECADB
